  ...
  select from DetalleAsiento {
    ...
    key cuentaContable.ID          as cuentaContable_ID : UUID,
        ...
        sum(case
            ...
            end)                   as importeDebe       : Decimal(15, 2),
        sum(case
              when clave = 50
                   then importe
              else 0
            end)                   as importeHaber      : Decimal(15, 2)
  }
  group by
    cabecera.ID,
    cuentaContable.ID,
...
@assert.unique: {cuentaContableUmbral: [cuentaContable], }
...
entity TipoAsiento : cuid, managed {
  ...
  subTipos   : Composition of many SubTipoAsiento
                 on subTipos.tipoAsiento = $self;
}
...
@assert.unique: {tipoAsientoConfigAdjunto: [tipoAsiento], }
